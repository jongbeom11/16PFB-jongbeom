...
def chk_bal(messsage, account):
    ...
    :return:
    """
    print ("%s : %d" % (messsage, account.check_balance()))
...
#CAshCard.py 모듈 안의 balance_won 값을 변환
...
chk_bal("mySisterCard_module 잔액 확인", mySisterCard_module)

# 그러나 이런 식으로는 한 장의 카드만 만들 수 있다.
print ("CashCard_module : %s" % CashCard_module)
print ("mySisterCard_module : %s" % mySisterCard_module)